...
The gilstep() function increments the states forward by 1 event. Which event happens first, and the time it takes for that event to happen are both random varaibles. We then need to do this many times. 
...
tau_df <- data_frame(S=array(Smat), I=array(Imat), R=array(Rmat), N=array(Nmat), cases=array(new_cases), time = rep(1:T,sims), iteration = rep(1:sims,each=T))
...
# put output in a data frame
bin_df <- data_frame(S=array(Smat), I=array(Imat), R=array(Rmat), N=array(matrix(Nmat)), time = rep(1:T,sims), iteration = rep(1:sims,each=T))
