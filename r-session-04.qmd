...
If we return to the code from R-session 1, we can plot a single deterministic realization of a model with $R_0 = 2$.
...
Now, perhaps we used the tools from R-session 3 to estimate $R_0$ and we think a reasonable $95\%$ confidence interval for $R_0$ is $(1.75,2.25)$. 
If we're quite certain the duration of infection is 6 days, then that means our corresponding confidence interval on the transmission rate is $(.291,.375)$.
...
for(m in 1:M){
  sir_params <- c(transmission = runif(1,.291,.375), duration = 6) # each time take a random draw of the transmission rate from the confidence interval
  sir_init_states <- c(S = 0.99, I = 0.01, R = 0)
  sim_times <- seq(0, 200, by = 0.1)
