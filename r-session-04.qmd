...
```{r}
num_iterations <- 100 # the number of times we want to do this.
# This is a mater of choice and computational capacity (this model is small and quick, but that won't always be the case)

#since we're going to do this num_iterations times we need a place to store the results
...
for (iter in 1:num_iterations) {
  ...
  sir_sol$iteration = iter # mark this as the iter iteration of the loop
...
This is because each event that occurs (e.g. someone getting infected or recovering) is analogous to a coin flip; even though the rules of the coin (the paramters) are always the same, the side it lands on is random.
...
But there some foundational versions of these models that illustrate the trade-offs between exact representaitons of the random processes we think are happening and the computational time it takes to generate outputs. For what we'll do here, everythind will be kind of fast, but in practice, for models of realistic scale, even a signle stochastic run can take a while. And if you have to do thousands of runs, even shaving a few seconds or minutes can be important.

## The Gillespie Algorithm

CHANGE THIS Text to motivate the Gillespie Algorithm
Explain the steps
...
gil_step <- function(SIR, beta, gamma) {
  ...
  times <- rexp(
    2, # draw two random exponential variables
    c(
      beta * SIR[1] * SIR[2] / sum(SIR[1:3]), # the first is the rate of new infections: S*Beta*I/N
      SIR[2] * gamma
    )
  ...
  return(list(
    ...
    time_step = min(times)
  )) # this identfies how much time elapsed before the transition occured, so we can increment time forward
}
```

The gil_step() function increments the states forward by 1 event. Which event happens first, and the time it takes for that event to happen are both random varaibles. We then need to do this many times.
...
while (all(I > 0)) {
  ...
  #
  sir_tmp <- c(S[counter], I[counter], R[counter], time[counter]) #current SIR states
...
for (iter in 1:num_iterations) {
  ...
  R <- 1 # number recovered
  time <- 0
  iteration = iter #initialize iteration counter
...
tau_sir_step <- function(sims, S, I, R, beta, gamma, delta_t, ...) {
  ...
  dSI <- rpois(n = sims, beta * S * (I / N) * delta_t) # new incident infections
  dIR <- rpois(n = sims, gamma * I * delta_t) # recoveries
  # note that this can be done with a binomial step as well
  #dSI <- rbinom(n=sims,size=S,prob=1-exp(-beta*(I/N)*delta_t))  # new incident infections
  #dIR <- rbinom(n=sims,size=I,prob=1-exp(-gamma*delta_t))       # recoveries
...
###############################################################################
max_time <- 100 # time to simulate over. With Tau Leaping the random draws scales with time not with population size, so this is much more efficient than Gillespie for large populations
...
for (time_step in 2:max_time) {
  ...
  i_mat <- rbind(i_mat, out[, 2]) # update state
  r_mat <- rbind(r_mat, out[, 3]) # update state
  n_mat <- rbind(n_mat, out[, 1] + out[, 2] + out[, 3]) # update state -- note population size isn't changing, but this could be updated with births/deaths
...
sir_colors <- c(S = "#1f77b4", I = "#ff7f0e", R = "#FF3851", cases = "#2ca02c") # fix this color
...
Note that the time series of new incident cases is very different than the time series of prevalent cases.
...
TO DO: ADD A CALLOUT HERE TO THE FACT THAT R0 IS NOT FIXED, IT DEPENDS ON THE PATHOGEN, THE POPULATION, AND THE MODEL.
...
chain_binomial_step <- function(sims, S, I, R, beta, ...) {
  ...
  N <- S + I + R # total population size
  new_i <- rbinom(n = sims, pmax(S, 0), 1 - exp(-beta * pmax(I, 0) / N)) # this is the only stochastic step; only do draw if I >0
  new_s <- S - new_i #this step is now deterministic, because everyone from the past time step recovers
  new_r <- R + I
...
max_time <- 20 # note here that the time step is one infectious generation time, so 7 days from gamma above
sims <- 1000
...
for (time_step in 2:max_time) {
  out <- chain_binomial_step(sims, s_mat[time_step - 1, ], i_mat[time_step - 1, ], r_mat[time_step - 1, ], beta)
  s_mat <- rbind(s_mat, out[, 1]) # update state
  i_mat <- rbind(i_mat, out[, 2]) # update state
  r_mat <- rbind(r_mat, out[, 3]) # update state
  n_mat <- rbind(n_mat, out[, 1] + out[, 2] + out[, 3]) # update state -- note population size isn't changing, but this could be updated with births/deaths
...
sir_colors <- c(S = "#1f77b4", I = "#ff7f0e", R = "#FF3851") # fix this color

# NOT CURRENTLY WORKING
# NOT PLOTTING AS INDIVIDUAL LINES
