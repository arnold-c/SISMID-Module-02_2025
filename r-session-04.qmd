...
ggplot(gil_df, aes(x = time, y = proportion, color = state)) +
  geom_line(linewidth = 1.5) +
  scale_color_manual(values = SIRcolors) +
  labs(
    x = "Time",
...
Because each realiztion is stochastic, we need to generate many runs to see the general behavior. 
...
sims <- 2    # number of simulations: notice that we can do WAY more now
...
SIRcolors <- c(S = "#1f77b4", I = "#ff7f0e", R = "#FF3851", cases = "green") # fix this color
...
#  filter(iteration==1) %>%
  ggplot( aes(x = time, y = number, color = state)) +
    geom_line(linewidth = 1.5,alpha = .1) +
    scale_color_manual(values = SIRcolors) +
    labs(
      x = "Time",
