...
Then, let's create a new vector `params0` that now has **4** initial values, for $\beta$, $X_0$, $Y_0$, **and** $\gamma$.
...
Here we modify the ``sse.sir`` function again to fix $X_0$ or $Y_0$ or both:
```{r}
#| code-fold: true
